...
def load_manifest(*, skip_manifest_download: bool) -> dict:
    ...
    if not path.is_file():
        if skip_manifest_download:
            raise FileNotFoundError(f'manifest.json is required for manifest-driven shard counts but is not present locally at {path}')
...
def main() -> None:
    ...
    if train_shards > max_train_shards:
        raise ValueError(f'{args.variant} only has {max_train_shards} training shards on {REPO_ID}, requested {train_shards}')
