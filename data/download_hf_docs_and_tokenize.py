...
def tokenizer_kind(spec: dict[str, Any]) -> str:
    ...
    if 'vocab_size' in spec:
        return 'sentencepiece_bpe'
    raise ValueError(f'unsupported tokenizer spec {spec.get("name", "<unnamed>")!r}: expected a built-in pure-byte or sentencepiece builder')
...
def build_tokenizers(
    *,
    ...
    for raw_spec in specs:
        ...
        recommended_bigram_vocab_size = int(built.get('recommended_bigram_vocab_size', ((vocab_size + 127) // 128) * 128 * 5))
        tokenizers.append(
            {
                'name': name,
                'kind': str(built['kind']),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Download docs_selected.jsonl from a Hugging Face dataset repo and tokenize it locally')
    parser.add_argument(
        '--repo-id',
